...
class ReleaseService:
    ...
    def run(self, part: str, *, push: bool, dry_run: bool) -> str:
        ...
        self.npm("install", "--package-lock-only")
        self.git("add", self.to_git_path(PACKAGE_FILE), self.to_git_path(LOCK_FILE))
        self.git("commit", "-m", f"chore: release cutie-logs {tag_name}")
    ...
    def get_current_version(self) -> str:
        ...
        if not isinstance(version, str):
            raise ReleaseError("Version was not found in package.json.")
        return version
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Bump cutie-logs version and create a release tag.")
